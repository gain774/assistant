...
## 障害時の対応
- `api.x.com` に 403 (proxy) → 実行環境のネット許可切れ。**未投稿分があれば send_later で30分後に自分を再発火させ、
  復旧していれば投稿する(最大3回=90分までリトライ)**。3回とも不通ならユーザーに1回だけ通知して終了。
  ネット許可が日中に外れる事象あり(2026-07-14)。予約発火時点で不通だと投稿が飛ぶためリトライで吸収する
